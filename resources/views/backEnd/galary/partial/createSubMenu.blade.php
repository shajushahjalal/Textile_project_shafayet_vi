<div class="modal fade" role="dialog" keyboard="false" data-backdrop="static" >
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        ...
        {!! Form::open(['url'=>'galary/sub-menu/create','id'=>'ajax-form','method'=>'post','files' => true]) !!}
        <div class="modal-body">
            <div class="form-group row">
                <div class="col-12 col-md-6">
                    <label>Menu Name <span class="text-danger">*</span></label>
                    <input type="hidden" name="id" value="{{isset($data->id)?$data->id:0}}">
                    <select class=" form-control" name="galary_menu_id" required>
                        <option value=""> Select Menu </option>
                        @foreach($galaryMenus as $menu)
                        <option value="{{$menu->id}}" {{isset($data->id) && $menu->id == $data->galary_menu_id ?'selected': null}} > {{$menu->menuName}} </option>
                        @endforeach
                    </select>
                </div>
                <div class="col-12 col-md-6">
                    <label>Galary sub-Menu <span class="text-danger">*</span></label>
                    <input class="form-control" name="subMenuName" placeholder="Sub-Menu Name" value="{{isset($data->id)?$data->subMenuName:null}}" >
                </div>
            </div>
            <div class="form-group row">
                <div class="col-12 col-md-6">
